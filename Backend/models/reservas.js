...
module.exports = {
  ...
  listado: async () => {
    const [rows] = await pool.query(
      'SELECT * FROM reservas WHERE state = ? ORDER BY fecha_reserva DESC, hora_reserva DESC',
      ['active']
    );
    return rows;
  },
    ...
    const [rows] = await pool.query(
      `SELECT hora_reserva 
       ...
       WHERE sucursal_id = ? 
         ...
         AND state = ?
         AND estado != ?`,
      [sucursalId, fechaReserva, 'active', 'cancelada']
    );
    return rows.map(row => row.hora_reserva);
  },
